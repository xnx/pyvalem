    ...
    def __init__(self, n, l=None, nocc=0, lletter=None):
        ...
        if l is None:
            ...
            try:
                ...
                raise AtomicOrbitalError('Invalid azimuthal quantum'
            ...
            try:
                ...
                raise AtomicOrbitalError('Invalid azimuthal quantum'
                                ' number, l={}. Must be in range 0 < l < {}'
                                .format(l, len(atomic_orbital_symbols)))
            if lletter and lletter != self.lletter:
                raise AtomicOrbitalError('Inconsistent azimuthal quantum'
                        ' number, l={} and label, lletter={} specified'
                        .format(l, lletter))
    ...
    def validate_atomic_orbital(self):
        if self.l > self.n - 1:
            raise AtomicConfigurationError('l >= n in atomic orbital {}'
                                                .format(self))
        if self.nocc < 0:
            raise AtomicConfigurationError('Negative nocc = {} not allowed in'
...
class AtomicConfiguration(State):
    ...
    def parse_state(self, state_str):
        ...
        try:
            ...
            raise AtomicConfigurationError('Invalid atomic electronic'
    ...
    def latex(self):
        latex_chunks = []
        if self.noble_gas_config:
            latex_chunks.append('\mathrm{{{}}}'.format(self.noble_gas_config))
        for orbital in self.orbitals:
    ...
    def _expand_noble_gas_config(self, config):
        ...
        if config[0] != '[':
            return config
        return (self._expand_noble_gas_config(noble_gas_configs[config[1:3]]) +
                config[4:])
