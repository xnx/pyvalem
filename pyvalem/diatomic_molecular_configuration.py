...
#                         )
alt_molecule_orbital = pp.Group(integer.setResultsName('n') +
                    pp.oneOf(atomic_orbital_symbols).setResultsName('lletter') + pp.Suppress('-')
                + pp.oneOf(molecular_orbital_symbols).setResultsName('symbol')
                + pp.Optional(integer.setResultsName('count'), default='1')
                               )
alt_molecule_config = (alt_molecule_orbital +
...
class DiatomicMolecularOrbital:
    ...
    def html(self):
        return '{:d}{:s}<sup>{:d}</sup>'.format(self.n,
                        self._make_symbol_html(self.symbol), self.count)
    ...
    def latex(self):
        return '{:d}{:s}^{{{:d}}}'.format(self.n,
                        self._make_symbol_latex(self.symbol), self.count)
...
class AltDiatomicMolecularOrbital(DiatomicMolecularOrbital):
    ...
    def __repr__(self):
        return '{}{}-{}{}'.format(self.n, self.lletter, self.symbol,
                                  self.count)
    __str__ = __repr__
    ...
    def __eq__(self, other):
        return (self.n == self.n and self.lletter == self.lletter and
                self.symbol == self.symbol and self.count == self.count)

    @property
    def html(self):
        return '{:d}{}{:s}<sup>{:d}</sup>'.format(self.n, self.lletter,
                        self._make_symbol_html(self.symbol), self.count)

    @property
    def latex(self):
        return '{:d}{}{:s}^{{{:d}}}'.format(self.n, self.lletter,
                        self._make_symbol_latex(self.symbol), self.count)
